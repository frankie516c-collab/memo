- 핵심: 폼 알아보기<br />
  url(http://127.0.0.1:5500/workspaces/WEB_STUDY-study/frontend/html/index.html)

- 내가 약한거: 태그

- 지환님 코드에서 CSS 응용하기
  - `<link rel="stylesheet" href="index.css">`: style 270줄 넘어가는걸 이렇게 연결
    - 목적: 가독성 (분리를 통해 코드가 너무 길어지는 것을 방지)
  - 선행작업: `index.css`를 만들어줘야함
     - `index.css`에는 모든 스타일이 다 포함된 코드를 넣음
     - `<style></style>` 내용 안에 있는 애들만 들어가면 됨
  - 홈페이지에서 F12 > Network > index.css > Response
